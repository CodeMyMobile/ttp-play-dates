  ...
  const metadataIndicatesJoined = useMemo(() => {
    ...
    const normalizeBooleanCandidate = (value) => {
      ...
      if (typeof value === "string") {
        ...
        return ["true", "1", "yes", "joined", "on_roster", "on-roster", "on roster"].includes(
          normalized,
        );
      }
      return false;
    ...
    const potentialSources = [
      ...
      matchData?.match,
      matchData?.metadata,
      matchData?.match?.metadata,
      matchData?.membership,
      matchData?.match_membership,
      matchData?.current_membership,
      matchData?.player,
      match,
    ];
    ...
    for (const source of potentialSources) {
      ...
      if (booleanCandidates.some(normalizeBooleanCandidate)) {
        return true;
      }
      ...
      if (
        statusCandidates.some((value) => {
          ...
          if (normalized.startsWith("joined")) return true;
          if (normalized.includes("on roster")) return true;
          return false;
        })
      ) {
        return true;
