...
import { createMatchGroup, deleteMatchGroup, getMatchGroup, updateMatchGroup } from "../services/matchGroups";
...
function GroupDetailPage({ groupId, onBack, onSaved }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try {
        setSearching(true);
        const data = await searchPlayers({ search: trimmed, perPage: 8 });
        if (cancelled) return;
        ...
        <div className="grid gap-5 lg:grid-cols-[1.3fr_1fr]">
              ...
              <div className="px-[18px] pb-[18px]">
                <div className="relative mb-4">
                  ...
                  {searchResults.length > 0 && (
                    <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-10 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                      {searchResults.map((player) => (
                        <button
                          key={player.id}
                          type="button"
                          onClick={() => addMember(player)}
                          className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm font-semibold text-slate-700 transition hover:bg-violet-50"
                        >
                ...
                {members.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-5 py-12 text-center">
                    ...
                    <div className="mt-3 text-sm font-bold text-slate-700">No members yet</div>
                  ...
                  <div className="space-y-3">
                    {members.map((member, index) => (
                      <div
                        key={member.id}
                        className={`flex items-center gap-3 py-2 ${index > 0 ? "border-t border-slate-100" : ""}`}
                      >
                        <PlayerAvatar
                          name={member.name}
          ...
          <aside className="self-start lg:sticky lg:top-24">
            <section className="mb-4 rounded-[14px] border border-violet-100 bg-violet-50/70 p-4">
              <div className="mb-3 text-[11px] font-black uppercase tracking-[0.18em] text-violet-500">
                Privacy
              </div>
              <p className="text-sm font-semibold leading-6 text-slate-700">
                Groups are private to you. Players do not see the group name, only the invite you
                send them.
              </p>
            </section>
